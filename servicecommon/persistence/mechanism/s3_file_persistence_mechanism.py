...
class S3FilePersistenceMechanism(AbstractPersistenceMechanism):
    ...
    def __init__(self, folder, base_name, must_exist=True,
                    bucket_base="", key_base="", logger=None):

        super(S3FilePersistenceMechanism, self).__init__(
                folder, base_name, must_exist, logger=logger)
    ...
    def open_source_for_read(self, read_to_fileobj,
        ...
        try:
            ...
            if self.logger is not None:
                self.logger.info("S3 file read {0} {1} succeeded",
                                    self.bucket_base, key)
            return_fileobj = 1
            ...
            if exception.response['Error']['Code'] == "404" \
                ...
                if self.logger is not None:
                    self.logger.info("S3 file read {0} {1} does not exist",
                                        self.bucket_base, key)
                return_fileobj = None
            else:
                ...
                if self.logger is not None:
                    self.logger.error(
                        "S3 file read {0} {1} some other error happened {2}",
                        self.bucket_base, key,
