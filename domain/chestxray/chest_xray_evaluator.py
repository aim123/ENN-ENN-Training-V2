...
# Custom imports
from domain.chestxray.auroc_history import AurocHistory
from domain.chestxray.data_gen_sequence import DataGenSequence
from domain.chestxray.chest_xray_subsampler import stratified_subsample
from domain.chestxray.keras_densenet import dense_net_121
...
NUM_IMAGES = [112120, 1121]


class ChestXrayEvaluator(KerasNetworkEvaluator):
    """
    ...
    def load_data(self, domain_config, data_pathdict):
        ...
        image_dir = 'chestxray_images'
        if not unzip_curr_dir:
            image_dir = os.path.join(domain_config.get('local_data_cachedir'),
                'chestxray/chestxray_images')
            image_dir = os.path.abspath(os.path.expanduser(image_dir))

        extract = True
        if not os.path.exists(image_dir):
            os.makedirs(image_dir)
        elif os.path.exists(image_dir) and \
            len(os.listdir(image_dir)) not in NUM_IMAGES:
        ...
        if extract:
            ...
            tar_command = "tar -xzf {0} -C {1}".format(datafile, image_dir)
    ...
    def test(self, model_evaluation, training_model, metrics,
        ...
        y_test = np.empty((len(partition[split]) -
                           ...
                           len(train_info['diseases'])), dtype=np.float32)
        ...
        for i in range(n_tests):
            ...
            if train_info['multitask']:
                ...
                for index in range(y_test.shape[0]):
                    for j in range(len(train_info['diseases'])):
        ...
        task_acc = []
        for i in range(len(train_info['diseases'])):
