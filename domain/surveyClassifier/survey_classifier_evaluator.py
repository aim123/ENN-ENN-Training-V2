...
#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ change start $$$$$$$$$$$$$$$$$$$$$$$$$
### added as per Elliot for multi-output
from keras.layers import Activation, Lambda

#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ change start $$$$$$$$$$$$$$$$$$$$$$$$$

class surveyClassifierEvaluator(KerasNetworkEvaluator):
    """
    ...
    def __init__(self):
        ...
        super(surveyClassifierEvaluator, self).__init__()
    ...
    def load_data(self, domain_config, data_pathdict):
        ...
        # Load data from file.   
        with open(data_pathdict['labels'], 'rb') as f:
          u = pickle._Unpickler(f)
          u.encoding = 'latin1'
          labels = u.load()
        with open(data_pathdict['tokens'], 'rb') as f:
          u = pickle._Unpickler(f)
          u.encoding = 'latin1'
          tokens = u.load()
        
        #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ change start $$$$$$$$$$$$$$$$$$$$$$$$
        # Convert single label tensor to 3 label tensor.
        for split in labels:
            y = np.array(labels[split]).transpose()
            labels[split] = [[np.array(i) for i in y[0]],[np.array(i) for i in y[1]],[np.array(i) for i in y[2]]]
    ...
    def evaluate_network(self, candidate_id, training_model,
        ...
        # Test the model
        fitness, task_fitness, metrics = self.test(domain_config, training_model, metrics,
                                                   data_dict)
    ...
    def create_training_model(self, core_model, domain_config):
        ...
        #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ change start $$$$$$$$$$$$$$$$$$$$$$$$
        # Split Final Layer into multiple output layers ... as per Elliot
        outputs = []
        start_idx = 0
        for num_classes in [12,220,107]:
          end_idx = start_idx + num_classes
          output = Lambda(lambda x: x[:,start_idx:end_idx])(core_model_output)
          softmax_output = Activation('softmax')(output)
          outputs.append(softmax_output)
          start_idx = end_idx

        # Instantiate full model.
        training_model = Model(inputs=my_input, outputs=outputs)
        print ("TRAINING MODEL SUMMARY ... ", training_model.summary())
    ...
    def test(self, domain_config, training_model, metrics, data_dict):
        """
        Called from evaluate_network() above
        ...
        #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ change start $$$$$$$$$$$$$$$$$$$$$$$
        #changes as per 3 node output 
        print("len of dev data", len((tokens['dev'],labels['dev'])))
	
        batch_size = domain_config.get('batch_size')

        loss_accuracy = training_model.evaluate(tokens['dev'],labels['dev'],batch_size=batch_size)
		
        fitness = float(np.mean([loss_accuracy[-1],loss_accuracy[-2],loss_accuracy[-3]]))
